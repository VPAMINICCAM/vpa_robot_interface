...
// IMPORTANT: robot_rotate_compensate_dir is only set to two values: +1 or -1. This indicates whether the robot's rotation angle compensation is in the positive direction or the negative direction.
// It relates to whether the gyro output angle increases or decreases when the robot's chassis rotates clockwise in practice. Therefore, please set it to either +1 or -1 based on the actual situation.
// From experience, when viewing the robot from above and it rotates clockwise, if the gyro data decreases, then this value should be set to +1; otherwise, it should be -1.
// Note that the gyro angle data computed in this SDK is in degrees, not radians.

const int robot_rotate_compensate_dir = +1;

// The starting and ending angles (in degrees) covered by the three pillars of the radar cover. Please set according to the actual installation situation.
...
// If there is no radar cover and no need to shield the radar cover, please set enable_cover to 0; otherwise, set it to 1.

const int enable_cover = 0;

// When the radar is installed on the robot chassis, the zero-point angular deviation between them (unit: degrees). Please set according to the actual installation
...
uint16_t stop=0xEF;
float PI = 3.1415926;
double angle_yaw_new = 0; //note: from gyro
double angle_yaw_old = 0; 
double zone_start_rotate_angle = 0; // Robot chassis rotation angle compensation starting offset value: 15 angular regions. When the radar turns to the start of a specific region, this is the angle rotated by the robot chassis.
...
{
	int fd;
	fd=open(port,O_RDWR | O_NOCTTY | O_NONBLOCK);// O_NONBLOCK is set to non-blocking mode.
	if(fd==-1)
	{
...
{ 
	...
	/* Save the existing serial port parameter settings. Error messages will be displayed here if there are issues with the serial port number, etc */
